...
impl Sequencer {
    ...
    /// Drop every lane, in every pattern, aimed at a device that has left
    /// `scope`'s chain. A channel's chain is only ever addressed from that
    /// channel's clips; a bus chain can be addressed from any of them.
    ///
    /// A reorder calls nothing here: a lane names the device.
    pub fn drop_lanes_for_device(&mut self, scope: EffectTarget, device: DeviceId) {
            ...
            match scope {
                EffectTarget::Channel(channel) => {
                    if let Some(channel) = pattern.channel_mut(channel as usize) {
                        channel.drop_lanes_for_device(scope, device);
                    }
                }
                EffectTarget::Bus(_) => {
