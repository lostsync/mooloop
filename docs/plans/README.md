...
| `session-layer-extraction/` | **Steps 01-02 done.** Six steps. Lifts a toolkit-free session crate out of `mooloop-ui/src/lib.rs` (13,411 lines, of which `UiState::new` is 7,700). Worth doing whether or not the egui plan ever runs; step 06 is the first test coverage the edit logic has ever had. |
