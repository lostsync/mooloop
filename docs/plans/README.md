...
| Plan | State |
| --- | --- |
| `latency-compensation/` | **Steps 02 and 03 in; 04 and 05 open.** `AUDIO_ARCHITECTURE.md`'s migration step 5, which its own header says steps 6 and 7 depend on — and step 6 is the typed audio edges `FOCUS.md` step 1 waits on. Adam settled the ordering on 2026-09-05: if compensation is on the path either way, building the edge type against an uncompensated tree means building it twice. A device now declares its latency without being built, and the tree compiles to a per-producer delay; nothing is compensated yet. |
