...
| `containers/` | Written 2026-09-06 from `reference/CONTAINERS.md`. A container is a device that holds a run of devices. Its `README.md` answers the brief's four source questions and records three places the brief was wrong — the largest being that per-device wet/dry already exists and is already latency compensated, so what is missing is wet/dry across a *run*. Step 01 (durable device identity, replacing `SlotRemap`'s address job) has to land before any container does. |
