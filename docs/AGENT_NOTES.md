...
What each model+harness pair did here, at more length than a roster row.
[CONTRIBUTORS.md](../CONTRIBUTORS.md) is the sign-in sheet — who worked on
this, when, and how often. This is where the detail went on 2026-09-14, once
that file's `Notes` field had grown to 70% of it; that file's "Why this is a
table" says what happened.

Nothing here is authoritative about the code. It is what each pair said about
its own work at the time, so it dates the way a diary dates: some of it
describes things that have since been rewritten, and a claim here is weaker
evidence than the commit it refers to. `docs/JOURNAL.md` is the narrative Adam
keeps; the source and the tests are the truth about current behaviour.

Append to your own pair's section if you want the detail kept. Length is not a
problem in this file. It is fine to trim your own section and nobody else's,
and it is fine to leave it alone — this one is allowed to be a diary.

## Claude Fable 5 — Claude Code

Wrote `docs/plans/modulator-modules/` — the modulator-grid plan —
and started step 01: modulator params join the descriptor system.

## Claude Fable 5.1 — Claude Code

Sanity pass over the sampler slice/commit push (commit bakes the
...
Parameter descriptors and the modulation design; seven of the
...
Rounded out the UI mockup tool's palette with the remaining real
...
Effect-container refactor: latency-aligned dry path, one dB trim
...
Audio-core architecture, realtime project swaps, compiled bus graphs,
...
Fixed duplicate loop-wrap event scheduling.

## Kimi k3-256k — Kimi Code CLI

Implemented the poly synth source device end to end (DSP voice pool,
engine integration, Slint face, and persistence).

## Kimi k3-256k — opencode

Implemented the effects-chain vertical slice (filter effect, end to end).

## ox-alpha — opencode

Rescued the ZoomScrollBar widget from an abandoned WIP branch and
