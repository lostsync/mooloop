...
Step 04 of `docs/plans/archive/poly-synth-v2/` had landed as parts: the LFO and the route compiler were written, tested and completely unread, every item carrying a dead-code allow. Wiring them into the sound is where the decisions were, and four of them are worth keeping.
...
Step 06 of `docs/plans/archive/poly-synth-v2/`, in two sittings: the vocabulary first, then the mechanism.
...
## Sep 5 (later) — a channel can hear another channel's oscillator

The debt the two instrument plans left, cleared in one push. ML-P8 step 06 and DS-01 step 07 both wanted the same thing — audio outlets somebody could actually read — and both had been left declared-but-unconnectable on purpose, because materialising seven stereo taps a channel for nobody would have been 7 MB across the bank for a feature that is off.

The plan's first decision was the one everything else fell out of, and it was forced rather than chosen. The control outlets take a block of latency: published at the end of a block, read in the next one, which makes graph order stop mattering and costs one block on a signal that only updates per block anyway. An audio edge cannot take that answer, because a block-sized delay is a delay whose length is the *host's buffer size* — so the same project would render differently at 128 and 512 frames, and `AUDIO_ARCHITECTURE.md` forbids that in as many words. Same-block delivery means the producer must render before the consumer, which means the channel loop needs a compiled order the way the bus walk already has one. **The plan is not a buffer. It is a schedule.** The buffer was easy once the order was decided.

Four things the doing turned up.

**A subscriber has to be a reason to run a source, and this is the trap.** ML-P8 skips an oscillator whose level is zero and which nothing else needs; DS-01 skips the body resonators the same way. Those are exactly the layers a *pre-level* tap exists to publish — the whole acceptance case is a muted `Osc 3` feeding another channel — so without folding tap demand into the same decision the patch already makes, the headline test would have published the zeros of a source that was never computed. It would have looked like a routing bug and been a skipping one. Any device implementing outlets later will meet this, which is why it is the first thing in the plan's status.

**Aux In had to publish its own output, for a reason that was not about Aux In.** ML-P8 and DS-01 never consume; a consumer that never produces makes a cycle impossible to construct from the program at all, so `compile_audio_graph`'s carefully-designed ring refusal would have been reachable only from a unit test. Publishing makes chains of Aux Ins work — an obvious thing to want on its own — and makes the acceptance case buildable. It also needed a new tap point: Aux In's whole content *is* its output, so `PreLevel` would republish exactly what it read, and `Output` could not be reused because it is deliberately the late tap that `TapIsLate` refuses. Weakening it to fit one device would have deleted a designed refusal.

**A byte of count is not a ceiling anybody chose.** `ParamCurve::Stepped(u8)` could not express a selector naming a channel: 256 channels plus "none" is 257 positions. Widening it to `u16` was ten mechanical edits and no behaviour, and it kept the plan's rule cheap — the parameter's value *is* the channel index, so there is no index-to-value mapping for a face to keep a second copy of, which is the trap `FOCUS.md` step 2 found the hard way on the v1 drum synth.

**A deleted producer is where the plan and the codebase disagreed.** The modulation rack deletes a route whose channel is deleted, and matching it was the obvious move. Step 04 asked for something else — the subscription inspectable and the consumer silent, "rather than pointing at whichever channel inherited the index" — so it is retained and pointed at the last addressable index, refused as `NoSuchChannel`, with the one corner (a song holding all 256 channels) written down beside the constant.
...
The mechanism is small — a tail in frames, a "my state has settled" flag, both defaulting to "never skip me" — and almost all of the work was in the four ways it is *not* small enough.

**Rest is about the node, not about the audio it was handed.** The plan suggested a memoryless effect could answer "have I seen non-silent input recently". It cannot: once asleep it sees nothing, so it could never say no again. The host counts input silence because it is the only side that can still count, and the node answers only for its own state. Which also turns the plan's `or` into an `and`: skipping on rest alone would sleep a bitcrusher under a full-scale signal, because a memoryless device's state is *always* settled.

**A frozen ring is not a running ring, twice over.** A sleeping node stops advancing its delay lines, so a delay time swept up afterwards reads audio from before the silence — every tail is therefore floored at the capacity of any ring a parameter can move a head inside, which for the delay is its whole two seconds. And then, much later and much harder to find: `DelayLine::read` takes its interpolation fraction from `write - 1 - offset`, so *where the write head sits* is part of the answer, and an `f32` holds fewer fractional bits at 2000 than at 200. A chorus that slept came back reading between different samples. The fix is two stores a frame; the better fix — take the fraction from the offset alone — is a real improvement to the primitive and was deliberately not made here, because it moves the output of the delay and the buffer device for reasons unrelated to this plan.

**Freezing is right for state driven by input and wrong for state driven by the clock.** "Waking must not reset state" was the plan's whole model and it is correct about filters and envelopes. It is silent about the reverb's line modulation, the chorus's LFO, the crusher's sample-and-hold counter and the poly synth's free-running LFO, all of which advance whether or not anything is sounding. Frozen, a hall came back somewhere else — and *how far* depended on the host's buffer size, which would have taken block-size-independent rendering with it, which is the property a bounce matching a take rests on. So there is a third method, `skip_block`, called in place of `process` for a block the host skips, and the implementations mirror the sample loop rather than closed-forming it: a phase that arrived by a different route is a different phase.

**Two devices make sound out of silence, and one of them is a feature.** Bitcrush's dither spans a whole quantiser step, so `quantize(0 + dither, step)` lands on plus or minus a step about half the time whatever the input was; at four bits that is -18 dBFS of hiss on a channel that has stopped playing. That is the device's noise floor rather than a tail, and it has a test. The retained-audio buffer plays back what it captured. Aux In's sound is another channel's and can start without an event of its own. All three decline the mechanism in writing, which is the difference between a decision and an omission.
...
## Sep 11 — the strip, and the knobs that could have lied

The console plan's last open step, and the one that had been waiting for the
other five: a channel strip on every track. Four sections -- an input stage,
four EQ bands, a compressor, a polarity switch -- all out by default, under
one strip-wide voicing, drawn on a new back face of the mixer strip and on a
pinned row in the track's device rack. `plans/archive/console/00-status.md`
has the whole account; three things from it are worth carrying here.

**The plan's own specification would have produced lying knobs.** It had a
voicing selecting *"knee, ratio law, attack and release curves, EQ band
frequencies and Q"* -- four voicings, each moving the numbers its own face
shows, so the 3 kHz under your finger is not the frequency being boosted. The
rule that replaced it is that **a voicing selects laws, never values**: the
harmonic profile, the tilt, the slew limit, the Q law, the curve above the
knee, the programme dependence. Every one of those is either invisible or
*drawn* -- by the response plot or the gain-computer curve -- which is the
condition that makes a law honest where a moved value is not. The Q law is
...
nothing: the whole descriptor table crosses into the markup once at startup
...
releases slower than a short one — was built with its second stage charging
off the *attack* time, twelve milliseconds at a one-millisecond attack. It
fills on a single transient, so the two cases released within three per cent
of each other, which is to say the mechanism was not there. Both its
constants are multiples of the *release* now, so "long" means long by the
standard the user set. The test had the same defect one level up: it compared
the RMS of a quiet tail, when what programme dependence changes is a
**time**. Counting blocks to recovery separates the cases by a factor of two,
with the uncoloured voicing as a control at one per cent — and it would have
caught the first version, which the level measurement did not.

Closing step 03 closed the plan. Step 06's preamp had landed two days before
it, measured against 106 plugins and given a device of its own so a rack had
somewhere to automate gain; what step 03 did was put it where the plan always
said it went. What is left of 06 is authoring rather than building — supply
sag, hysteresis, a fitted tilt — and each is a row of a table and a pair of
...
**A label is a promise, and the way out of a bad promise is sometimes to stop making it.** The channel strip refused selectable EQ frequencies because a printed hertz value would have to be one voicing's, and `Iron` wanting 2.2 kHz where `Moo` wants 2.5 kHz would make the label lie on three faces out of four. Adam removed the premise rather than the feature: *"i say we dont label. we just have N positions/band and they're selectable. tooltip/statusbar can carry the specific frequency information to the user on hover."* A band stores a **position** now, and `StripEqTable` gives each voicing its own hertz for each position. This is the strip's own "a voicing selects laws, never values" rule arrived at from the other end — the number a knob shows must be one no voicing can move, and a position index is such a number — and it is worth noticing that the rule had been used for a whole plan to *forbid* something, when read the other way round it permitted it.

**An overlay that is not an overlay.** Two 20x16 buttons placed in a `Rectangle` at `y: root.height - 20px` — an overlay by every appearance — added their height to what the sibling `VerticalLayout` already needed and pushed the last row out of the bottom of the face. It cost most of an afternoon and four wrong theories, each of which was *plausible and testable*: a meter's segment count, a fader's `min-height`, the scroll bar's width, a column's `alignment: start`. Two of the four turned out to be real bugs on their own (a `ToolButton` floor that thirty-one sites were silently getting, and a fader floor 20px higher than it needed) and fixing them changed nothing about the symptom, which is what made them so convincing. The lesson is the procedure, not the fact: when a layout overflows and the arithmetic says it should not, **delete the absolutely-positioned children first**, and instrument rather than reason — a render plus a few lines of PIL reading one pixel column was decisive in seconds where four rounds of careful arithmetic were wrong.

**A constraint cannot be relaxed by the place that uses it.** Slint takes the stricter of a component's `min-height` and an instance's, so `ToolButton`'s 24px floor beat every one of the thirty-one sites asking for less. None of them failed; they all just silently got 24, and the sizes in the markup read as though they were in effect. The fix was to lower the floor in the widget and give it a *preferred* size instead, which is what those sites had actually been expressing all along.
...
Routing was the second. The cell was shared by every generation, and nothing republished it after an install, so a channel removal, move, paste or undo left later channels reading another channel's input until the port list next changed. Republishing it after the install would still have left a block or two in which the outgoing renderer read the incoming channel order. `InputState` now carries the incoming project's routing (`Session::project_midi_routing`, resolved in `install_project_in_ui`), the incoming renderer gets a routing cell of its own, and the handle's later writes go to that cell. The test failed red with the note on the outgoing project's channel.
...
## Sep 20 — looking at a pattern was an act of restructuring the music

Adam: *"when i change what pattern im looking at, the audio glitches. i'm wondering what the fundamental issue is that keeps causing that."* He suspected the renderer being rebuilt, which is the glitch `incremental-structure/` closed. It was not that. Pattern selection installs nothing; it sends a POD `SetCurrentPattern`, and the command sets `seeked`, which makes the block loop push `Event::Choke` onto every live channel at offset 0. On MonoSynth, PolySynth, ML-M1 and ML-P8 a choke is `release_all`, which is precisely the symptom he described — *"a long synth will stop holding that note."*

The finding that turned it from a trade-off into a bug was his, not the code's: *"its really only an issue in song mode."* Every read of `Sequencer::current` that reaches playback is inside a `PlaybackMode::Pattern` arm — `schedule_pattern`, `schedule_once`, `automation_lane_at`, `has_automation_at`. Song mode schedules from playlist placements and never looks at it. So in Song mode the command changed nothing that was scheduled and then killed every voice anyway. Three more cases were free of charge the same way: a switch with the transport stopped, a re-selection of the pattern already current, and a selection past the end of the bank. `set_current_pattern` answers `bool` now and `apply_command` pays only when the selection moved, the mode is Pattern and the transport is running.

All four tests were run against the unfixed tree first and reported a peak of `0` against `0.25` before the switch — failing because the voice was killed rather than because nothing was sounding, which is the distinction that makes a red run worth doing. They measure the master's peak rather than the event lists: a `Choke` that is emitted and then not reached would pass an event-list check, and `release: 0.0` on an ML-P8 is a fast fade rather than an instant one, so "still audible a block later" would have passed straight through the bug.

**The near-miss is worth more than the fix.** The first draft put both debts the command owes behind one condition, `transport.playing` included, because they read alike: let go of the notes, hand back the knobs. They are not alike. `process` resolves automation lanes whether or not the transport is running, deliberately — *"so that a knob does not jump the moment you press play"* — so a paused switch off an automated pattern parks the destination exactly as a running one does, which is the hazard `restore_lanes_left_behind` was written for on Sep 14. One condition would have fixed a loud bug by reintroducing a quiet one. It came out of re-reading the diff rather than out of a test, and the test that now pins it had to be written twice: the first version asserted against a transport that had never run, where the strip is asleep, nothing had resolved and there was nothing to strand. It passed nothing and proved nothing, and the only reason that was visible is that it had been written with an assertion saying so — *"the lane has to be resolving while stopped, or this test proves nothing"*. Guard clauses in tests earn their keep on the day the setup is wrong.

**The fault underneath is that the control plane has two granularities and no third.** Events are sample-accurate and carry an offset; commands are drained at the top of the block and applied immediately, with no offset and no target tick. There is no way to say *at the next musical boundary*, and no way to say *over the next N frames*. And `seeked` is the engine's only vocabulary for *the program changed*, so a pattern switch borrowed a seek's blunt global recovery. That is why the previous fixes never stopped the class: `Smoothed` on the audible continuous parameters, an attack that starts from the current level, the equal-power crossfades, choke-instead-of-mute, `restore_lanes_left_behind` — each closed one door, while discontinuity stayed the default and continuity stayed opt-in per site. `AUDIO_ARCHITECTURE.md` had already written down the missing half: *reset and transport discontinuities are not* in the node contract.

`plans/archive/transport-discontinuity/` carries the rest — a command that lands at a musical edge, an `AudioNode` that can be told time moved, and the rule the whole thing serves, which is Adam's sentence: *"i just want to be able to move around the app freely without having audio issues."* He ruled on the two open questions in four words — *"immediate and yes"* — so the pattern selector keeps switching now rather than queueing, and a stopped transport stops releasing. The first of those retired step 02's own justification before it was built: queueing was going to be what removed the Pattern-mode choke, and with immediate chosen, nothing does, because under a running transport in Pattern mode the release is genuinely owed. The step stays, for the granularity rather than for this bug, and says so.
...
- ~~ML-P8 stops inside step 06~~ — closed 2026-09-05, along with DS-01's step 07 and `typed-audio-edges/` itself. All three directories are archived. What was *not* built is the rest of `AUDIO_ARCHITECTURE.md`'s step 6. **Parallel sends landed 2026-09-09** (`plans/archive/console/` step 05) and did it without extending this mechanism: a send carries its own compensation, so it is aligned by construction and never asks `compile_audio_graph` anything. **Sidechain key inputs** are still open — they need a dependency edge that schedules a producer without summing it in — and now extend a compiled edge model rather than needing one built first.
...
- ~~Mixer: inserts only, no sends, sidechain, solo, stem export, or bus renaming.~~ Sends landed 2026-09-09 and tracks can be renamed and removed as of the same day. The channel strip landed 2026-09-11, so EQ and compression are on every track rather than on inserts only. Solo landed 2026-09-11, in place rather than as the monitor tap `archive/MIXER_PLAN.md` had specified since the mixer's first pass. What is left of this thread is **no sidechain and no per-track stem export**, plus two pieces of the send work that are stage 2: a tap point below pre-fader (after a named device, or at one of its declared outlets), and a send authored from a *channel* — the engine compiles one, but the mixer draws no channel strips for the control to live on. An `Aux In` channel reads another channel's published outlet as of 2026-09-05, which is an input to a device rather than a send: the producing channel does not know it is being read and its own routing does not move.
...
- ~~Channels have no colour — no field in the UI, the session model, or the project format — which is the one genuinely new persisted thing the planned channel sidebar needs.~~ Closed 2026-09-13 (`color-picker.slint`, `3661ef9`): a channel, a track and a pattern each take a colour the song owns rather than an index into the theme, so a song looks the same under every scheme. This bullet had simply been left behind, which is the failure the keyboard-focus entry above is also an example of — a gaps list is only worth reading if closing a gap includes striking it. **The sidebar's swatch grid became a chip beside the name field on 2026-09-15**, sharing `ColorChipButton` with the pattern field in the transport toolbar; the swatches moved into its popup.
