...
the detailed audio-core contract in `AUDIO_ARCHITECTURE.md` separate.
...
- `mooloop-core` is the dependency-light source of truth for musical and
  project state. The UI, engine, and persistence crate share its model rather
  than translating between private representations.
- `mooloop-ui` owns the interactive editor. It sends compact commands and
  receives events and meter data through `EngineHandle`; it does not execute
  DSP itself.
- `mooloop-engine` separates preparation from the audio callback. Expensive
...
state is reclaimed on the control thread. See `AUDIO_ARCHITECTURE.md` for the
full contract and planned evolution of compiled routing, latency compensation,
and future sends or sidechains.
